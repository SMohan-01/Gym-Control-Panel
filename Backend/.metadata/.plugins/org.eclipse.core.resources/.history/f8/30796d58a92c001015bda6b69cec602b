...
public class MemberService implements IMemberService {
	...
	public MemberEntity updateMember(Long id, MemberEntity updatedMember) throws MemberNotExistException {
		...
		if (fetchedMember.isEmpty()) {
			...
			if (!existing.getMembershipType().equalsIgnoreCase(updatedMember.getMembershipType())) {
				LocalDate newStartDate = LocalDate.now();
				LocalDate newEndDate = calculateMembershipEndDate(newStartDate, updatedMember.getMembershipType());
				existing.setMembershipStartDate(newStartDate);
				existing.setMembershipEndDate(newEndDate);
	...
	@Override
	public boolean deleteMember(Long id) throws MemberNotExistException {
		Optional<MemberEntity> fetchedMember = memberRepository.findById(id);
	...
	@Override
	public MemberEntity renewMembershipPlan(Long id, String membershipType) throws MemberNotExistException {
		Optional<MemberEntity> fetchedMember = memberRepository.findById(id);
		System.out.println(membershipType);
		if (fetchedMember.isEmpty()) {
			throw new MemberNotExistException("Member Not Exist");
			...
			MemberEntity member = fetchedMember.get();
			LocalDate newStartDate = LocalDate.now();
	        LocalDate newEndDate = calculateMembershipEndDate(newStartDate, membershipType);

	        member.setMembershipType(membershipType);
	        member.setMembershipStartDate(newStartDate);
	        member.setMembershipEndDate(newEndDate);
	        member.setActive(true);
	        member.setAmountPaid(true);
	...
	public void deactivateExpiredMembers() {
	    ...
	    List<MemberEntity> expiredMembers = memberRepository.findByMembershipEndDateBeforeAndActiveTrue(today);
	    
	    for (MemberEntity member : expiredMembers) {
	        member.setActive(false);
	        member.setAmountPaid(false);
	        memberRepository.save(member);
	    }
	}

	@Override
	public List<MemberEntity> getExpiredMembers(){
		LocalDate today = LocalDate.now();
		deactivateExpiredMembers();
	    return memberRepository.findByMembershipEndDateBefore(today);
	}
}
